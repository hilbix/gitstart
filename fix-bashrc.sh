...
grep -qw PS1 "$TMP" ||
cat <<'EOF' >> "$TMP"
[ -z "$PS1" ] || { __i=; for __a in /etc/bash_completion.d/git /usr/share/doc/git-*/contrib/completion/git-completion.bash; do [ -r "$__a" ] && __i="$__a"; done; [ -n "$__i" ] && . "$__i" && PS1="`echo "$PS1" | sed -e '/__git_ps1/b' -e 's|\(\\\\\\\$[[:space:]]*\)$|\$(__git_ps1 "(%s)")\1|'`"; unset __a __i; }
EOF
